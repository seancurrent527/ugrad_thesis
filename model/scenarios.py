...
class Scenario:
    def __init__(self, name, network=None, outfile=None, **kwargs):
        ...
        Population.network = self.network
        ...
        self.populations = [Populus(st) for st in self.states]
        print(*[pop.name for pop in self.populations], sep = ',', file = self.outfile)
    ...
    def write_out(self):
        print(*[pop.population for pop in self.populations], sep = ',', file = self.outfile)
    ...
    def run(self, timesteps = 100):
        for t in tqdm(range(timesteps)):
            for pop in self.populations:
                ...
                pop.recalculate()
            self.write_out()
            migrations = [pop.migrants for pop in self.populations]
